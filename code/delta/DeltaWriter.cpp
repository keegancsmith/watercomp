...
DeltaWriter::DeltaWriter(FILE* output_file, bool output)
...
void DeltaWriter::start(int atoms, int frames, int ISTART, int NSAVC, double DELTA)
{
...
/* Writes a frame */
void DeltaWriter::next_frame(const QuantisedFrame& qframe)
{
    ...
    if(frames.size() == 1)
    {
        ...
        {
            int predicted = frames[0].quantised_frame[i];
