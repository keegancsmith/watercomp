...
int main(int argc, char ** argv) {
    ...
    float _min_coord1[3] = { std::numeric_limits<float>::max() };
    float _max_coord1[3] = { std::numeric_limits<float>::min() };
    float _min_coord2[3] = { std::numeric_limits<float>::max() };
    float _max_coord2[3] = { std::numeric_limits<float>::min() };
    float * min_coord1 = dynamic_quant ? _min_coord1 : 0;
    float * max_coord1 = dynamic_quant ? _max_coord1 : 0;
    float * min_coord2 = dynamic_quant ? _min_coord2 : 0;
    float * max_coord2 = dynamic_quant ? _max_coord2 : 0;
    ...
    for (size_t frame = 1; frame <= reader1.nframes(); frame++) {
        ...
        QuantisedFrame q1(f1, quantx, quanty, quantz, min_coord1, max_coord1);
        QuantisedFrame q2(f2, quantx, quanty, quantz, min_coord2, max_coord2);

        if (dynamic_quant) {
            for (int d = 0; d < 3; d++) {
                min_coord1[d] = q1.min_coord[d];
                max_coord1[d] = q1.max_coord[d];
                min_coord2[d] = q2.min_coord[d];
                max_coord2[d] = q2.max_coord[d];
            }
        }
